...
```bash
ipfs init             # create repo in ~/.ipfs
ipfs daemon           # start the node
ipfs add myfile.txt   # add; kubo pins by default unless --pin=false
ipfs pin add <cid>    # pin explicitly if you disabled auto-pin on add
```

- Local endpoints: WebUI/API at `http://localhost:5001/ipfs` (also `.../webui`), local gateway/explorer at `http://127.0.0.1:8080/ipfs`.
- To let others fetch data you pinned on your device, the node must be reachable (NAT traversal/hole punching or open ports). Without that, your CID may stay local-only unless a reachable peer also pins it.
